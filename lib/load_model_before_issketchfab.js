        ...
        await loader.load('./PHP/3D_models/' + modelName.slice(0, -4) + '.glb', async function(gltf) {
            ...
            await fetch('./json/' + modelName.slice(0, -4) + '.json?nocache=' + new Date().getTime())
                ...
                .then(function (data) {
                    ...
                    const userTitle = prompt('Enter a title for the narrative:', 'AR-default');
            ...
            function runApp() {
                            ...
                            if (intersects.length > 0) {
                                ...
                                try {
                                    ...
                                    if (changeCoordinates) {
                                        ...
                                        createAnnotationDOM(text, '', numberOfCanvas + 1, coordinates, false, annotationListDiv);

                                        // create new canvas
                                        let newAnnotationCanvas = createCanvas(numberOfCanvas);

                                        createSprite(newAnnotationCanvas, coordinates, numberOfCanvas, false);
                ...
                annotationListTemp.forEach((ann, index) => {
                    ...
                    // create DOM element
                    createAnnotationDOM(ann.title, ann.description, index + 1, ann.coordinates, ann.isSketchfab, annotationListDiv);
                    ...
                    let spriteFront = createSprite(annotationCanvas, ann.coordinates, index, ann.isSketchfab);
...
function reloadAnnotations(annotations) {
    ...
    if (annotations) {
        ...
        annotations.forEach(function (ann) {
            ...
            for (let i = 0; i < events.length; i++) {
                if ('annotationNumber3DModel' in events[i][1] && parseInt(events[i][1].annotationNumber3DModel) === index + 1) {
                    let description = events[i][1].text.text;  // events[i][1].text?.text || events[i][1].description
    ...
    annotationListTemp.forEach((ann, index) => {
        ...
        // Sprites - circles and numbers
        createSprite(annotationCanvas, ann.coordinates, index, ann.isSketchfab);
...
function computeAnnotationScale(referenceObject, isSketchfab) {
    ...
    return referenceSize * multiplier;
}
...
function createSprite(canvas, point, number, isSketchfab) {
    ...
    let newScale;
    isSketchfab
        ? newScale = modelSize*0.01
        : newScale = modelSize*0.04;*/

    const annotationScale = computeAnnotationScale(model, isSketchfab);
    ...
    spriteFront.scale.set(annotationScale, annotationScale, annotationScale);
    ...
    spriteRear.scale.set(annotationScale, annotationScale, annotationScale);
...
function createAnnotationDOM(title, description, index, position, isSketchfab, annotationListDiv) {
