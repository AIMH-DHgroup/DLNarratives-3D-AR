...
if (!chosenModel) {
                ...
                .then(function (data) {
                    ...
                    for (let property in data["events"]) {
                        ...
                        if (Object.keys(data["events"][property]).includes('sketchfabid')) {
                            ...
                            event_digobj[index] = data["events"][property]["objurl"];
                            event_props[index] = new Map(Object.entries(data["events"][property]["props"])); // keep props order
                        }
                    }
                            ...
                            if (!err) {
                                ...
                                for (let property in json_data["events"]) {
                                    if (property.includes('customAnnotation')) {
                                        ...
                                        const position = json_data["events"][property]["_position"];
                                        ...
                                        if (isCustomAnn) {
                                            ...
                                            addAnnotationToList(title, position, coordinates, event_ID[index]);
                                        } else {
                                            // add annotation button in annotationsPanel
    ...
    function addAnnotationToList(text, i, point, event_ID) {
        ...
        editButton.addEventListener('click', function (e) {
            ...
            try {
                ...
                fillForm(title, description, coordinates, position, digobjurl, digobjtitle, digobjtable, entities, entitiesDiv, point, i, oldPosition, eventID, eventName, false);
    ...
    function fillForm(title, description, coordinates, position, digobjurl, digobjtitle, digobjtable, entities, entitiesDiv, point, positionInList, oldPositionInList, eventID, eventName, initializeProps) {
        ...
        const digobj = event_digobj[positionInList - 1];
        ...
        if (initializeProps) {
            ...
            props_temp = new Map([...event_props[positionInList - 1]]);
            ...
            if (keys && keys.length > 0) {
                for (let i = 0; i < keys.length; i++) {
                    ...
                    addEntity(thisEntity, entityTitle, entityClass, keys[i]);
                }
            }
...
/*function initializeEmptyValues(obj) {
    if (Array.isArray(obj)) {
        // If it is an array, return an empty array with the same length
        return obj.map(() => null);
    } else if (obj !== null && typeof obj === 'object') {
        // If the key is 'props', initialize it as an empty object
        if ('props' in obj) {
            obj.props = {};
        }
        // Process each key
        return Object.fromEntries(
            Object.entries(obj).map(([key, value]) => [
                key,
                key === 'props' ? {} : initializeEmptyValues(value)
            ])
        );
    }
...
function saveJSONAnnotations() {
    try {
        ...
        if (typeof events[eventID] === undefined) events[eventID] = {
            "_id": "",
            "end": "",
            "date": "",
            "text": {
            ...
            "position": null,   // refers to all kind of events - SMBVT compatible
            "unique_id": "",
            "eventMedia": "",
            "eventVideo": "",
            "start_date": {
            ...
            "eventMediaCaption": "",
            "eventVideoCaption": "",
            "annotationNumber3DModel": "",  // refers to the number of the annotation of models loaded from sketchfab
            'coordinates': null
        };
        ...
        events[eventID].text.text = description; // with HTML code
        ...
        // props
        const index = document.getElementById('old-position').textContent;
        event_props[index - 1] = props_temp;
        events[eventID].props = Object.fromEntries(event_props[index - 1]);
        props_temp = new Map;
        ...
        // TODO - update all 'annotation-index' values and 2d/3d objects also
        ...
        event_digobj[oldPosition - 1] = []; // reset digital objects of the event
        if (arrayDigObj.length > 0) {
            for (let i = 0; i < arrayDigObj.length; i++) {
                event_digobj[oldPosition - 1].push({
                    'url': arrayDigObj[i].url,
                    'title': arrayDigObj[i].title
                });
            }
        ...
        if (position !== oldPosition) {
            ...
            for (let i = 0; i < entries.length; i++) {
                if (eventID === String(entries[i][0])) {
                    if (entries[i][1].position !== position) {
                        for (let j=0; j < entries.length; j++) {
                            if (entries[j][1].position === position) {
                                const temp = entries[j][1];
                                events[temp._id] = entries[i][1];
                                events[entries[i][1]._id] = temp;
                            }
                        }
                    }
                }
...
function addEntity(props, text, type, id) {
...
if (document.readyState === "complete") {
    ...
    function addSelectedItem(item) {
        sparqlRequest(item, true, undefined).then(result => {
            ...
            addEntity(result, item.value, typeFromArray(result.type), result._id);
            ...
            /*if (event_props[position - 1] !== undefined) {
                const mergedProps = new Map(event_props[position - 1]);
                newEntityMap.forEach((value, key) => {
                    mergedProps.set(key, value);
                });
                event_props[position - 1] = mergedProps;
            } else {
                // otherwise push the new entity as a new event
                event_props.push(newEntityMap);
            }*/
